...
(function webpackUniversalModuleDefinition (root, factory) {
	if (typeof exports === 'object' && typeof module === 'object') { module.exports = factory() } else if (typeof define === 'function' && define.amd) { define([], factory) } else if (typeof exports === 'object') { exports.Raphael = factory() } else { root.Raphael = factory() }
})(window, function () {
...
: eve.on("raphael.DOMload", function () {
    ...
    let loaded
        const separator = /[, ]+/
        const elements = { circle: 1, rect: 1, path: 1, ellipse: 1, text: 1, image: 1 }
        const formatrg = /\{(\d+)\}/g
        ...
        const supportsTouch = ('ontouchstart' in window) || window.TouchEvent || window.DocumentTouch && document instanceof DocumentTouch
        const E = ""
        const S = " "
        const Str = String
        const split = "split"
        ...
        const tCommand = /([rstm])[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029,]*((-?\d*\.?\d*(?:e[\-+]?\d+)?[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029]*,?[\x09\x0a\x0b\x0c\x0d\x20\xa0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u3000\u2028\u2029]*)+)/ig
 ...
 if (abs(values[i] - value) <= tolerance) {
    ...
    let toHex = function (color) {
        if (R.vml) {
            ...
            let bod
            try {
                const docum = new ActiveXObject("htmlfile")
                docum.write("<body>")
                docum.close()
                bod = docum.body
            } catch (e) {
            ...
            const range = bod.createTextRange()
...
? C / (2 * L)
 ...
 if (array[i] === item) {
    ...
    R.getColor = function (value) {
        const start = this.getColor.start = this.getColor.start || { h: 0, s: 1, b: value || 0.75 }
 ...
 if (p[has](key) && key != ps) {
    ...
    function interPathHelper (path1, path2, justCount) {
        ...
        let x1; let y1; let x2; let y2; let x1m; let y1m; let x2m; let y2m; let bez1; let bez2
            let res = justCount ? 0 : []
    ...
    const pathDimensions = R.pathBBox = function (path) {
        ...
        var path2curve = R._path2curve = cacher(function (path, path2) {
            ...
            const p = pathToAbsolute(path)
                ...
                const processPath = function (path, d, pcom) {
                    let nx; let ny; const tq = { T: 1, Q: 1 }
                    if (!path) {
        ...
        const parseDots = R._parseDots = cacher(function (gradient) {
            const dots = []
    ...
    (function (matrixproto) {
        ...
        matrixproto.add = function (a, b, c, d, e, f) {
            const out = [[], [], []]
                const m = [[this.a, this.c, this.e], [this.b, this.d, this.f], [0, 0, 1]]
                let matrix = [[a, c, e], [b, d, f], [0, 0, 1]]
                let x; let y; let z; let res
 ...
 if (key[has](i)) {
    ...
    elproto.drag = function (onmove, onstart, onend, move_scope, start_scope, end_scope) {
        function start (e) {
            ...
            if (supportsTouch && e.touches) {
                let i = e.touches.length; let touch
                while (i--) {
                    touch = e.touches[i]
            ...
            drag.push({ el: this, move_scope, start_scope, end_scope })
 ...
 if (draggable[i].el == this) {
    ...
    const getOffset = function (elem) {
        const box = elem.getBoundingClientRect()
            ...
            const clientTop = docElem.clientTop || body.clientTop || 0; const clientLeft = docElem.clientLeft || body.clientLeft || 0
    ...
    const getLengthFactory = function (istotal, subpath) {
        return function (path, length, onlystart) {
            path = path2curve(path)
            let x; let y; let p; let l; let sp = ""; const subpaths = {}; let point
                let len = 0
            for (let i = 0, ii = path.length; i < ii; i++) {
                p = path[i]
                if (p[0] == "M") {
...
/*! no static exports found */
/***/ function (module, exports, __webpack_require__) {
let __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__; !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! ./raphael.core */ "./dev/raphael.core.js")], __WEBPACK_AMD_DEFINE_RESULT__ = (function (R) {
    if (R && !R.svg) {
        return
    }
...
: {
 ...
 if (markerCounter[has](attr) && !markerCounter[attr]) {
    ...
    const addDashes = function (o, value, params) {
        ...
        if (value) {
            const width = o.attrs["stroke-width"] || "1"
                const butt = { round: width, square: width, butt: 0 }[o.attrs["stroke-linecap"] || params["stroke-linecap"]] || 0
                const dashes = []
                let i = value.length
            while (i--) {
                ...
                switch (att) {
                    ...
                    case "clip-rect":
                        ...
                        if (!value) {
                            ...
                            if (path) {
                                const clip = R._g.doc.getElementById(path.replace(/(^url\(#|\)$)/g, E))
    ...
    var tuneText = function (el, params) {
        ...
        const a = el.attrs
            const node = el.node
            const fontSize = node.firstChild ? toInt(R._g.doc.defaultView.getComputedStyle(node.firstChild, E).getPropertyValue("font-size"), 10) : 10
 ...
 if (par[has](subkey)) {
    ...
    R._engine.rect = function (svg, x, y, w, h, r) {
        ...
        svg.canvas && svg.canvas.appendChild(el)
        const res = new Element(el, svg)
        res.attrs = { x, y, width: w, height: h, rx: r || 0, ry: r || 0, fill: "none", stroke: "#000" }
        res.type = "rect"
        $(el, res.attrs)
        return res
    }
...
/*! no static exports found */
/***/ function (module, exports, __webpack_require__) {
let __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;// Copyright (c) 2013 Adobe Systems Incorporated. All rights reserved.
//
...
? (module.exports = eve)
: (true
? (!(__WEBPACK_AMD_DEFINE_ARRAY__ = [], __WEBPACK_AMD_DEFINE_RESULT__ = (function () { return eve }.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__)),
				__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__)))
: (undefined))
})(this)
/***/ }
